<template>
  <div class="mt-6">
    ...
    <div v-if="selectedOption!=='لطفا یک گزینه را انتخاب نمایید.'">
      <v-text-field ref="title" v-model="title" dense label="عنوان" outlined placeholder="عنوان"/>

      <suggest-text v-model="text" :rules="rules.text" :title-input="$refs.title" label="متن پیامک"/>

      <v-textarea v-if="selectedOption==='درخواست مشاوره متن'" v-model="description" class="mt-4" dense label="توضیحات"
                  outlined placeholder="توضیحات"/>

      <v-row class="mt-5">
        <v-btn :disabled="validate" :loading="loading" class="btn-get-code" style="letter-spacing: normal"
               @click="submit">
          ثبت درخواست
        </v-btn>
      </v-row>
  ...
  methods: {
    submit() {
      ...
      userStoreItem({
        ...
        type: this.selectedOption === 'درخواست مشاوره متن' ? 'request' : 'generate',
      }).then(response => {
