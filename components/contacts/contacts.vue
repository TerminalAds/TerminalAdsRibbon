    ...
    <template v-if="!!tagList.length">
      <v-window v-model="selectedTag" vertical>
        <v-window-item v-for="n in tagList" :key="n.id">
          <v-card
            class="phone-list-bg rounded-lg position-relative"
            flat
            height="120"
          >
            <v-row
            ...
            >
              ...
              <div class="text-center white--text">
                <h2>{{ n.name }}</h2>
                <span v-if="isSharedPhonebook" class="ma-2 font-small">{{
                  i18n.t("shared")
                }}</span>
              </div>
            </v-row>
      ...
      <v-card
        :disabled="loading || loadPhones"
        :loading="loading || loadPhones"
        class="d-flex justify-center justify-md-space-between pa-2"
        flat
      >
        <v-btn
        ...
        <div class="d-flex align-start">
          ...
          >
            <v-btn
              :color="tagList.length ? '#FF4400' : 'success'"
              class="mx-1"
              dark
              depressed
              fab
            ...
            >
              <v-icon
                >mdi-{{ tagList.length ? "dots-vertical" : "plus" }}</v-icon
              >
            </v-btn>
          </contacts-tags>
          ...
          <v-item-group v-model="selectedTag" class="tag-items" mandatory>
            ...
            >
              <v-btn
                ...
                @click="selectItem(i)"
              >
                <v-icon v-if="n.hasOwnProperty('owner_id')" left
            ...
            <v-item class="mx-1 mb-2">
              <v-btn
                color="success"
                outlined
                @click="$refs.contactTags.dialog = true"
              >
          ...
          <v-tab-item>
            <v-card-title class="font-size-h1">
              ...
              <span
                ...
                class="font-size-h6 mx-1 grey--text"
                >({{ data.total }})</span
              >
              <v-spacer />
              <contacts-menu
            ...
            <contacts-table
              ...
              :data="data"
              :loading.sync="loadPhones"
              :phone-tag="tagList[selectedTag]"
              :table-props.sync="tableProps"
              @reload="reloadTable"
            />
            <!--                            @select="selectRow"/>-->
          ...
          <template v-if="hasPhonebookPermission('add')">
            ...
            <v-tab-item>
              <contacts-number-view
                ...
                v-model="data"
                :loading="loading || loadPhones"
                :props.sync="tableProps"
                @addBulk="importFromBulk"
                @changePage="(e) => fetchContacts({ ...tableProps, page: e })"
                @reload="reloadTable"
              />
            </v-tab-item>
            ...
            <v-tab-item>
              <contact-import-from-excel
                v-if="!!tagList && !!tagList[selectedTag]"
                :tags="tagList[selectedTag].id"
                @fetch="
                  () => {
                    reloadTable({ ...tableProps, page: 1 });
                    tabs = 0;
                  }
                "
              />
    ...
    fetchTags() {
      ...
      this.$payamakAxios
        ...
        .then(({ data }) => {
          ...
          } else this.setActivePhonebook(this.selectedTag);
