<template>
  <v-card :loading="loading" flat>
    <v-card-title class="white" style="position: sticky; top: 80px;z-index: 1">
      ...
      <v-spacer/>
      <v-btn v-if="!hasQuery && hasPhonebookPermission('edit')" class="btn-edit" depressed
             @click="goto(`phonebook/edit/${info.phone}?change=${info.name}`)">
        <v-icon left>mdi-pencil-outline</v-icon>
        <span class="letter-spacing-none">ویرایش مخاطب</span>
      </v-btn>

      <v-btn v-else :disabled="!canSave" class="btn-edit" depressed @click="saveEdited">
        <v-icon left>mdi-check</v-icon>
        <span class="letter-spacing-none">ثبت ویرایش</span>
      </v-btn>

      <template v-if="!hasQuery && hasPhonebookPermission('delete')">
        <v-btn :loading="loading" class="btn-delete ma-0" depressed @click="deleteContact">
      ...
      <v-card-text>
        <v-list-item>
          ...
          <v-list-item-content>
            ...
            <v-list-item-subtitle class="font-size-h3 mt-1" v-text="info.companyName || ''"/>
          </v-list-item-content>
        </v-list-item>
      </v-card-text>
        ...
        <v-col :md="popup ? 12 : 6" cols="12">
          ...
          <v-card-text class="pb-0">
            <div v-for="(email, i) in info.email" :key="i" class="mb-3">
              ...
              <a :href="`mailto:${email.email}`" class="text-primary">{{ email.email }}</a>
              <span v-if="email.tag" class="px-1 grey--text"> . {{ email.tag }}</span>
            </div>

            <div class="mb-2">
              <v-icon right>mdi-phone-outline</v-icon>
              <a :href="`tel:${info.phone}`" class="text-primary">{{ info.phone }}</a>
            </div>
            ...
            <div class="mb-2">
              ...
              <span v-if="!info.birthday" class="text-primary cursor-pointer"
                    @click="goto(`phonebook/edit/${info.phone}?change=${info.name}`)">افزودن تاریخ تولد</span>
              <span v-else>{{ info.birthday | showBirthday }}</span>
            </div>

            <div v-for="(website, i) in info.website" :key="i" class="mb-2 cursor-pointer">
              <v-icon right>mdi-link</v-icon>
              <span v-if="!!website.tag" class="px-1 grey--text">{{ website.tag }} . </span>
              <a :href="website.link" class="text-primary" target="_blank">{{ website.link }}</a>
            </div>
          </v-card-text>

          <div v-if="!!info.description" class="px-4 pb-4 d-flex align-start">
            <v-icon class="mt-2">mdi-note-outline</v-icon>
            <pre class="flex py-2 px-4 ms-2 grey lighten-2 rounded-lg" v-html="info.description"/>
          </div>
        </v-col>
        ...
        <v-col :md="popup ? 12 : 6" cols="12">
          ...
          <v-card-text class="d-flex">
            <template v-if="!!info.phone_tags && !!info.phone_tags.length">
              <v-chip v-for="(tag, i) in info.phone_tags" :key="i" class="rounded-lg mx-1" close outlined
                      @click:close="deleteContactTag(tag.id)">
                <span class="font-size-h6 grey--text font-smaller">{{ tag.name }}</span>
              </v-chip>
            </template>

            <v-chip v-else :disabled="loading" class="rounded-lg" outlined>
              <span class="font-size-h6 grey--text font-smaller">no tag/tags</span>
            </v-chip>
          ...
          <v-card-text>
            <div class="mb-2">
              آخرین ویرایش
              <span class="px-1 grey--text"> . {{ info.updated_at | timeToNow }}</span>
            </div>

            <div class="mb-2">
              به مخاطبین اضافه شده
              <span class="px-1 grey--text"> . {{ info.created_at | timeToNow }}</span>
            </div>
          </v-card-text>
        </v-col>
      </v-row>
    ...
    <contacts-edit-fields v-else ref="editContact" v-model="info" :avatarFile.sync="avatarFile" :can-more.sync="canSave"
                          :popup="popup"/>
  </v-card>
</template>
...
export default {
  ...
  filters: {
    ...
    showBirthday(str) {
      ...
      if (date.length > 2) {
        month = moment().jMonth(Number(date[1]) - 1).startOf('jMonth').format('jMMMM')
        return date[0] + ' ' + month + ' ' + Number(date[2])
      }
      month = moment().jMonth(Number(date[0]) - 1).startOf('jMonth').format('jMMMM')
  ...
  computed: {
    ...
    fromAndroid() {
      return !!this.$route.query && !!Object.keys(this.$route.query).length && this.$route.query.from === 'android'
    }
  },
  ...
  methods: {
    getInfo() {
      ...
      this.$payamakAxios.get(`contact/${this.slug}`)
        .then(({data}) => {
          let d = data.data
          ...
          try {
            this.info = {
              ...d,
              address: !!d.address ? typeof d.address === 'string' ? JSON.parse(d.address) : d.address : [],
              custom: !!d.custom ? typeof d.address === 'string' ? JSON.parse(d.custom) : d.custom : [],
              email: !!d.email ? typeof d.address === 'string' ? JSON.parse(d.email) : d.email : [],
              website: !!d.website ? typeof d.address === 'string' ? JSON.parse(d.website) : d.website : [],
              date_times: !!d.date_times ? typeof d.address === 'string' ? JSON.parse(d.date_times) : d.date_times : []
            }
          } catch (e) {
            console.log(e)
            this.info = {
    ...
    saveEdited() {
      ...
      arr.forEach(val => {
        if (val[0] === 'phone_tags')
          ...
          formData.append(val[0], JSON.stringify(this.filterFormDateListItem(val[1])))
        else
