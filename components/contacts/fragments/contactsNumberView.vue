  ...
  <v-card flat>
    <v-card-title class="align-center">
      <v-card class="rounded-pill d-flex justify-center align-center pa-2" outlined>
        ...
        <v-btn icon @click="() => {expandedSearch = !expandedSearch; clearSearch()}">
    ...
    <template v-if="(!!value.data && !!value.data.length) || (!!bulk && !!bulk.length)">
      <v-row class="pa-4" justify="center" no-gutters>
        <template v-if="!bulk || !bulk.length">
          <contacts-number-edit v-for="(phone, i) in value.data" :key="i" :value="phone" @delete="deleteNumbers"/>
        </template>

        <template v-else>
          <contacts-number-edit v-for="(phone, i) in computedBulk.list" :key="i" :value="phone" prepare
                                @delete="deleteNumbers($event, true)"/>
        </template>

        <v-col v-if="!bulk || !bulk.length" class="pa-2 d-flex justify-center align-center" cols="12">
          <template v-if="!!value && !!value.last_page && value.last_page > 1">
            <v-pagination v-model="value.current_page" :length="value.last_page" circle total-visible="5"
        ...
        <v-col v-else class="pa-2 d-flex justify-center align-center" cols="12">
          <template v-if="computedLastPage > 1">
            <v-pagination v-model="paginate.current_page" :length="computedLastPage" circle total-visible="5"/>
          </template>
        </v-col>
      </v-row>
    </template>
...
export default {
  ...
  data() {
    return {
      ...
      phoneRule: {
        phone: (v) => isValidPhone(v) || v.length === 0 || this.$t("ERRORS.PhoneIsNotValid")
      },
      search: ''
    }
  },
  ...
  computed: {
    ...
    computedBulk() {
      ...
      let arr = this.bulk.filter(item => !this.search || item.includes(this.search));
      return {
        len: arr.length,
        list: arr.slice((current_page - 1) * per_page, ((current_page - 1) * per_page) + per_page)
      }
    },
    computedLastPage() {
  ...
  methods: {
    importFromClipboard(value) {
      ...
      filterCorrectPhoneNumbers(phone)
        .then((validNumbers) => {
    ...
    getFileContent() {
      ...
      switch (type) {
        case 'xlsx':
          readXlsx(file)
            .then((data) => this.extractPhoneNumbers(data))
          break;
        case 'txt':
        case 'csv':
          readFile(file, (data) => {
