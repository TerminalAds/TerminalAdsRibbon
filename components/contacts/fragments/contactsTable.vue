<template>
  <div>
    <vue-table v-model="selected" :data="data" :headers="headers" :loading.sync="computedLoading"
               :props.sync="computedProps" class="contact-table" filter-on-enter show-select @change="onPagination"
      ...
      <template v-slot:filters="{ props, perPage }">
        <v-row no-gutters>
          ...
          <v-col class="pa-2" cols="12" lg="3" md="4">
            <v-select v-model="props.tag" :items="fieldTagsItems" :loading="fieldTagsLoading"
                      :menu-props="{offsetY: true}" chips class="rounded-lg" clearable dense hide-details
                      item-value="key" label="عنوان" multiple outlined @change="changeFieldTag(props.tag)">
              <template v-slot:item="{item, on, attrs}">
                <v-list-item v-bind="attrs" v-on="on">
                  <v-list-item-icon>
                    <v-icon>mdi-checkbox-{{ attrs.inputValue ? 'marked' : 'blank-outline' }}</v-icon>
                  </v-list-item-icon>
          ...
          <v-slide-x-reverse-transition>
            <v-col v-if="hasSearchTag" class="pa-2" cols="12" lg="3" md="4">
              <v-text-field v-model="props.search_tag" append-icon="mdi-tag-hidden" class="rounded-lg" clearable dense
                            hide-details label="جستجو در عنوان" outlined
                            @keydown.enter="onPagination({['search_tag']: props.search_tag})"/>
            </v-col>
          </v-slide-x-reverse-transition>

          <v-slide-x-reverse-transition>
            <v-col v-if="hasDateTag" class="pa-2 d-flex align-center" cols="12" lg="3" md="4">
              <custom-date-picker v-model="props[`datetimes_start`]"
                                  :label="`${selectedFiledTag.find(s => s.type === 'datetimes').text} از`"
                                  append-icon="mdi-calendar-month"
                                  class="flex" dense format="YYYY-MM-DD" hide-details input-class="datetime-start"
                                  nullable outlined type="date" wrapper-class="rounded-lg"
                                  @input="onPagination({['datetimes_start']: props[`datetimes_start`]})"/>
          ...
          <v-slide-x-reverse-transition>
            <v-col v-if="hasDateTag" class="pa-2 d-flex align-center" cols="12" lg="3" md="4">
              <custom-date-picker v-model="props['datetimes_end']"
                                  :label="`تا ${selectedFiledTag.find(s => s.type === 'datetimes').text}`"
                                  append-icon="mdi-calendar-month"
                                  class="flex" dense format="YYYY-MM-DD" hide-details input-class="datetime-end"
                                  nullable outlined type="date" wrapper-class="rounded-lg"
          ...
          <v-col class="pa-2 d-flex align-center" cols="12" lg="3" md="4">
            <custom-date-picker v-model="props.birthday_start" :max="maxStart" append-icon="mdi-cake-variant-outline"
                                class="flex" dense format="jYYYY-jMM-jDD" hide-details input-class="bd-start"
                                label="تاریخ تولد از" nullable outlined type="date" wrapper-class="rounded-lg"
                                @input="onPagination({['birthday_start']: props.birthday_start})"/>
            <!--                                @input="changeBirthday(props)"/>-->
          </v-col>

          <v-col class="pa-2 d-flex align-center" cols="12" lg="3" md="4">
            <custom-date-picker v-model="props.birthday_end" :max="maxEnd" :min="props.birthday_start"
                                append-icon="mdi-cake-variant-outline" class="flex" dense format="jYYYY-jMM-jDD"
                                hide-details input-class="bd-end" label="تا تاریخ" nullable outlined type="date"
          ...
          <v-col class="pa-2" cols="12" lg="1" md="2">
            <v-select v-model="props.length" :items="perPage" :menu-props="{offsetY: true}" class="rounded-lg"
                      dense hide-details label="تعداد" outlined @change="onPagination({['length']: props.length})"/>
          </v-col>
        </v-row>
      </template>

      <template v-slot:image="{item}">
        <img :src="item.avatar || '/media/users/blank.png'" alt="" class="pa-1 overflow-hidden rounded-pill"
             style="object-fit: cover;width: 44px;height: 44px"/>
      </template>

      <template v-slot:name="{item}">
        {{ !item.name && !item.lastName ? '-' : (item.name || '') + ' ' + (item.lastName || '') }}
      </template>
      ...
      <template v-slot:phone="{item}">
        ...
        <v-tooltip top>
          <template v-slot:activator="{on, attr}">
            <v-btn class="mx-1" icon small v-bind="attr" v-on="on"
      ...
      <template v-slot:title="{item}">
        {{ !item.jobTitle && !item.companyName ? '-' : (item.jobTitle || '') + ', ' + (item.companyName || '') }}
      </template>
      ...
      <template v-slot:tag="{item}">
        <div class="d-flex flex-wrap align-center" style="gap: 4px">
          ...
          <template v-for="(tag, i) in item.phone_tags">
            <v-chip v-if="!tag.parent_id" :key="i" class="rounded-lg" outlined small>
              {{ tag.name }}
            </v-chip>
          </template>
        </div>
      ...
      <template v-slot:action="{item}">
        <contacts-buttons :data="item" :tag="phoneTag" @delete="showDeleteDialog"/>
      </template>
    </vue-table>
...
export default {
  ...
  components: {ContactSendsPattern, ContactsButtons, CustomDatePicker, UserPicker, CustomPopup, VueTable},
  ...
  data() {
    return {
      ...
      deleteCons: {
        title: 'حذف مخاطب',
        buttons: [
          {type: 'submit', handler: () => this.deleteItem()}
        ]
      }
    }
  },
  ...
  created() {
    ...
    if (ribbonCan('contact_monitoring') && this.$route.name === 'admin.contact.monitoring')
  ...
  computed: {
    ...
    maxStart() {
      ...
      return !!this.tableProps.birthday_end ? this.tableProps.birthday_end : moment().format('YYYY-MM-DD')
    },
    maxEnd() {
      const moment = require('moment')
    ...
    hasSearchTag() {
      return !!this.selectedFiledTag?.filter(item => item.type.indexOf('date') < 0)?.length
    },
    hasDateTag() {
      return !!this.selectedFiledTag?.filter(item => item.type.indexOf('date') >= 0)?.length
    }
  ...
  methods: {
    ...
    handleAction(e, state = 'default') {
      if (state !== "default") {
        let els = document.getElementsByClassName('laravel-vue-datatable-tbody-tr');
        if (!!els) {
    ...
    deleteItem() {
      ...
      deleteContactFromTag({
        ...
        .catch(({response}) => {
          this.$toast.error(response?.data?.message || 'خطا در حذف مخاطب از دفترچه تلفن!')
        })
    ...
    changeFieldTag(value) {
      ...
      this.selectedFiledTag = this.fieldTagsItems.filter(item => value.includes(item.value + '_' + item.type));
