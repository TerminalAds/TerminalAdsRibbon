<template>
  <custom-popup v-model="dialog" :cons="{title: 'انتخاب پترن/کمپین'}">
    <template v-slot:activator>
      <v-btn :class="!!value && !!Object.keys(value).length ? 'btn-edit' : 'btn-refund'" :loading="loading" class="ma-0"
             style="letter-spacing: normal">
        <v-icon class="me-2">mdi-plus</v-icon>
        <span v-if="!!value && !!Object.keys(value).length">تغییر پترن/کمپین</span>
    ...
    <v-card :loading="loading" class="pa-4" flat>
      <v-card-title class="pa-2">
        <v-btn :href="$router.resolve({ name: 'text.user.list' }).href" class="btn-refund">
          <v-icon class="me-2">mdi-plus</v-icon>
          ثبت پترن جدید
        </v-btn>

        <v-btn :href="$router.resolve({ name: 'user.campaign' }).href" class="btn-send-sms">
          <v-icon class="me-2">mdi-plus</v-icon>
          ثبت کمپین جدید
        </v-btn>
      </v-card-title>

      <custom-tabs v-model="tabs" :tab-items="[{text: 'انتخاب پترن'}, {text: 'انتخاب کمپین'}]" grow>
        <template v-slot:staticItems>
          <v-tab-item class="py-4">
            <data-table :columns="columns" :data="data" :per-page="[10, 25, 50, 100, 400]" :translate="translate"
                        @row-clicked="selectRow" @on-table-props-changed="reloadTable"/>

            <v-card v-if="(!data.data || !data.data.length) && !loading"
                    class="text-muted text-center font-size-h3 d-flex align-center justify-center" flat height="200">
...
export default {
  ...
  data: () => ({
    ...
    columns: [
      ...
      },
      {
        ...
        name: 'active',
        transform: ({data}) => {
          return data.active === 0 ? `<p class="badge badge-danger">رد شده</p>`
  ...
  mounted() {
    ...
    if (queries && typeof queries === "object" && Object.keys(queries).length > 0) {
      if (!!queries.link && !!queries.pattern) {
        ...
        this.getData({...this.tableProps, Uid: queries.pattern})
          .then(() => {
            ...
            setTimeout(() => {
              this.computedValue = this.data?.data?.find(item => item.id == queries.pattern)
              this.$nextTick(() => {
  ...
  methods: {
    ...
    selectCamp(data) {
      const obj = this.$refs["message-list"]?.data?.data.find(item => item.id === data)
      delete obj.id
