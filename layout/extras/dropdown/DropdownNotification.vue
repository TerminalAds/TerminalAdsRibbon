<template>
  <div>
    <v-img
      ...
      contain
      max-width="350"
      src="../../../assets/img/communication/bg-1.jpg"
    >
    ...
    <b-tabs v-model="tabIndex" class="hide-tabs">
      <b-tab active class="pa-2">
          ...
          <template>
            ...
            >
              <v-list-item-icon class="rounded-lg me-2">
                <img
                  alt=""
                  src="../../../assets/img/communication/Mail-notification.svg"
                />
              </v-list-item-icon>
        ...
        <v-card-actions class="justify-end pa-0">
          <v-btn
            color="primary"
            :href="`${$coreApi}/#/user/notifications`"
            small
            target="_blank"
            text
          >
      ...
      <v-card v-if="dialogData && dialogData.data" flat>
        ...
        <v-card-text v-if="ribbon_can('admin_access')">
          <div v-if="containsKey(dialogData.data, 'customerHistory')">
            <v-btn
              @click="
                goToCustomerHistory(dialogData.data.customerHistory.phone)
              "
              >{{ i18n.t("go_to_user") }}</v-btn
            >
          </div>
        </v-card-text>
...
export default {
  ...
  methods: {
    ...
    seenDialog() {
      this.dialog = false;
      this.$DashboardAxios
        .post("/api/core/seen/notification", { notif_id: this.dialogData.id })
        .then(({ data }) => {
          this.getNotifications();
        });
    },
