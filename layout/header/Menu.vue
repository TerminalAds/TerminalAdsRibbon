<template>
  <div>
    <ul class="menu-nav has-background">
      <li class="menu-item exact-color" @click="closeMenuOnClick()">
        ...
        >
          <span class="menu-text"
            ><i class="fas fa-home fa-2x home-icon-color"></i
          ></span>
        </a>
      </li>
      ...
      <div
        v-else-if="
          (!sectionStatus.menus || !topMenus || !topMenus.length) && !hasMenu
        "
      ...
      <template v-else>
        <li
          v-for="(menu, i) in topMenus"
          :key="i"
          :class="[
            !menu.parent_id
              ? 'menu-item '
              : 'menu-item menu-item menu-item-submenu menu-item-open-dropdown',
            { 'menu-item-active': hasActiveChildren(menu.slug) },
          ]"
          :data-menu-toggle="!menu.parent_id ? 'hover' : 'click'"
          aria-haspopup="true"
        >
          <a
            v-if="!menu.children && menu.slug.startsWith('http')"
            :href="menu.slug"
            class="menu-link"
            target="_blank"
          >
          ...
          <v-menu v-else close-on-click offset-y open-on-hover>
            <template v-slot:activator="{ on, attrs }">
              <v-btn
                class="menu-link font-weight-normal"
                dark
                text
                v-bind="attrs"
                v-on="on"
...
body[data-scrolltop="on"]
  .has-background
  :not(.exact-color i, .menu-submenu *) {
